...
脚本会自动：
1. 检查并安装 PyInstaller
2. 检查根入口 `main.py` 和 `src/app` 包，并把 `src` 加入 PyInstaller 包搜索路径
3. 收集 `app` 子模块；根目录只保留统一启动入口 `main.py`
...
源码运行入口为根目录 `main.py`；实际 GUI、CLI、应用服务、领域逻辑、错误反馈和适配器实现位于 `src/app/`。
...
### Q: 打包失败？
- 查看错误信息，最常见的是依赖缺失
- 先确认能直接运行：`python main.py -args gui`，能跑通再打包
